...
io.on("connection", (socket) => {
    ...
    socket.on("requestMatchMake", (data) => {
        ...
        socket.emit("matchMakeSuccess", {p1: "매치된 플레이어 1", p2: "매치된 플레이어 2"}, '방이름');
    ...
    //api :플레이어가 매치후 레디 버튼을 누르면 2번 올것이다. 
    socket.on("requestStart", (data) => {
        socket.emit("startGame");//api: 시작 신호 보내기. 서버에서 쓰레드 돌리기 시작, if문으로 구별해서 보내기
        gameStart();//게임 실행 함수 호출
        //api: 각각의 방마다 따로 돌아야 한다.이부분을 공부해 보자
